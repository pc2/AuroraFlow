...
extern "C"
{
    ...
    void send_data(
        ...
        hls::stream<ap_uint<1>, 1> &ack_stream
    ) {
    send_iterations:
        for (unsigned int n = 0; n < iterations; n++) {
        send_chunks:
...
#pragma HLS dataflow
        ...
        send_data(iterations, chunks, frame_size, data_input_stream, ring_output, ack_stream);

        recv_data(iterations, chunks, data_output_stream, ring_input, ack_stream);
